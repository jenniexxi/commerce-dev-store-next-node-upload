...
import { AvailableCouponList, CouponList, CouponType, OrderItem } from 'type/api/order';
import { CustomCartItem } from 'app/order/cart/ShoppingCart';
import { CouponTypeCode, CouponTypeCodes } from 'type/promotions';
import { T } from '@ui/commons';
import { colors } from '@ui/styles/theme';
import { numberWithCommas, showPriceText, showShippingPriceText } from '@ui/utils';
import Selector, { Option } from '@ui/components/selector/Selector';
import { CartsList } from 'type/api/shoppingCart';
import { Button, Checkbox, Modal } from '@ui/components';
import { useGetOrderCouponList } from 'hooks/query/promotionQuery';
...
const CouponInfoView = ({ coupon, doubleCoupon, storeCoupon, orderData, cartId }: Props) => {
  ...
  const { data: orderCouponList, refetch: refetchCoupon } = useGetOrderCouponList(cartId, showCouponListModal);
  ...
  const getCouponOptions = (cartId: number) => {
    ...
    const options: Option<string>[] = [
      ...
      ...(dupCoupons.length > 0 && selectedCoupons[cartId]?.type === CouponTypeCodes.Duplication
  ...
  const handleCouponSelect = (cartId: number, value: string) => {
    ...
    const [type, code] = value.split('_') as [CouponTypeCode, string];

    // 선택된 쿠폰 찾기
    const selectedCouponList =
      type === CouponTypeCodes.Goods
        ? coupon?.availableCouponList
        : type === CouponTypeCodes.Duplication
                                      ...
                                      {hasAvailableCoupons(coupon, goods.cartId) && (
                                        <div className='discount-row'>
                                          ...
                                          <Selector
                                            ...
                                            defaultValue={
                                              selectedCoupons[goods.cartId]?.type === 'COUPON.TYPE.GOODS'
                                        ...
                                        selectedCoupons[goods.cartId]?.type === 'COUPON.TYPE.GOODS' && (
                                          <div className='discount-row'>
                                            ...
                                            <Selector
                                              ...
                                              defaultValue={
                                                selectedCoupons[goods.cartId]?.type === 'COUPON.TYPE.DUPLICATION'
