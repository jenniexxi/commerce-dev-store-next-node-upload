...
import OrderClaimPopup, { Popup } from './features/OrderClaimPopup';
import OrderProductGroup from './features/OrderProductGroup';
import TooltipBox from './features/TooTipBox';
import { useHeader } from '@ui/hooks';
import { useRouter } from 'next/navigation';
import navigate from 'utils/navigate';
import { ClaimType } from 'type/api/claim';
import { AllOrderStates, CLAIM_TYPE_CODES, ItemStateKey } from 'type/claims';
import { useModalStore } from '@ui/stores';
import { PAYMENT_STATUS_CODES, paymentMethodCode } from 'type/orders';
import { Button } from '@ui/components';
import { DeliveryAddress } from 'type/api/buyer';

import { showPriceText, showShippingPriceText } from '@ui/utils';
import { ClaimList, OrderHistoryDetailShippingList } from 'type/api/order';
import { useGetOrderListDetail } from 'hooks/query/orderQuery';
import { useConfirmOrder, useGetDeliveryTracking } from 'hooks/mutate/orderMutation';

type Props = {
  ordersIdEncrypt: string;
};
const OrderDetail = ({ ordersIdEncrypt }: Props) => {
  ...
  const { data, refetch } = useGetOrderListDetail(ordersIdEncrypt);

  const { mutate: confirmOrder } = useConfirmOrder((resp) => {});

  const { mutate: getTrackingInfo } = useGetDeliveryTracking((resp) => {
    resp.data.url + resp.data.invoiceNumber;
  });

  const moveToClaimOrder = (isSelectOrder: boolean, type: ClaimType, orderItemIdEncrypt?: string) => {
    router.push(
      ...
      <S.OrderHiDetailWrapper>
        ...
        <S.OhDetailSecPay>
          ...
          <S.AmountPaidPart>
            ...
            <S.PointBox>
              <S.PointTitle>적립예정 라운드페이 포인트</S.PointTitle>
            ...
            <S.ExpectedPoints>
              ...
              <li>구매확정 이후 적립될 라운드페이 포인트는 해당 예상 라운드페이 포인트 내역과 상이할 수 있습니다.</li>
            </S.ExpectedPoints>
          </S.AmountPaidPart>
        </S.OhDetailSecPay>
        <S.OhDetailSecShip>
        ...
        <S.OhDetailSecBtn>
          ...
          />
          {data?.data.claimList &&
            data?.data.claimList.filter((claim: ClaimList) => claim.claimTypeEnum.code === CLAIM_TYPE_CODES.Cancel)
              .length > 0 && (
              <Button
                title='취소 환불내역'
                btnType='tertiary'
          ...
          {data?.data.claimList &&
            data?.data.claimList.filter((claim: ClaimList) => claim.claimTypeEnum.code === CLAIM_TYPE_CODES.Return)
              .length > 0 && (
              <Button
                title='반품 환불내역'
                btnType='tertiary'
