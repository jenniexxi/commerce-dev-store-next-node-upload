...
import { CouponListResp } from 'type/api/promotion';
...
import { CouponTypeCodes } from 'type/promotions';
import { Button, Modal, NonModalTooltip } from '@ui/components';
import { showMileageText } from '@ui/utils';
...
const ProductPointInfo = ({ goodsInfo, goodsId, goodsPriceInfo, couponData, myPageInfo, refetch }: Props) => {
  ...
  const { mutate: downloadCoupon } = useDownloadCoupon((data) => {
    if (data.success) {
      refetch();
    } else {
          ...
          <S.PointSummaryViewItem>
            <S.ToolTipView>
              ...
              <NonModalTooltip
                ...
                withDot={true}
                items={[
                  '라운드페이 포인트는 구매 확정 시 적립됩니다',
                  '적립 금액은 쿠폰 사용, 라운드페이 포인트 사용 및 상품 옵션 금액을 기준으로 적립되므로 최종 적립 금액은 쿠폰사용여부 및 옵션 금액에 따라 달라질 수 있습니다',
                  '라운드페이 포인트 사용 금액에 대한 적립은 제공되지 않습니다',
            ...
            <S.PointSummaryViewItem>
              <S.ToolTipView>
                ...
                <NonModalTooltip
                  ...
                  withDot={true}
                  items={[
                    '결제 적립은 라운드페이머니 결제 등, 특정 조건에서 결제 시 적립이 가능합니다',
              ...
              <S.RowViewBetween className='flex-item sub-item'>
                <S.ToolTipView className='sTitle'>
                  <p className='tipTitle'>라운드페이 머니 결제 시 최대 적립</p>
                  <NonModalTooltip
                    title='라운드페이 머니 결제 시 최대 적립'
                    trigerColor={colors.icon4}
                    trigerType='!'
                    position='center'
                    showCloseButton={true}
